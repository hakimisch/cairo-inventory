...
function AddMaintenanceForm({ assets, onDone }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('assets.maintenances.store', data.asset_id), {
            preserveScroll: true,
    ...
    return (
        <form onSubmit={handleSubmit} style={{ padding: 16, background: '#FFF7ED', borderRadius: 8, border: `1px solid #FED7AA`, marginBottom: 16 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#C2410C', marginBottom: 12 }}>+ Penyelenggaraan Baru</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                ...
                <div>
                    ...
                    <input type="date" style={inputStyle} value={data.maintenance_date} onChange={e => setData('maintenance_date', e.target.value)} required />
                </div>
                <div>
                    <label style={labelStyle}>Syarikat</label>
                    <input type="text" style={inputStyle} value={data.company_name} onChange={e => setData('company_name', e.target.value)} placeholder="Nama syarikat..." />
                </div>
                <div>
                    <label style={labelStyle}>No. Kontrak</label>
                    <input type="text" style={inputStyle} value={data.contract_no} onChange={e => setData('contract_no', e.target.value)} placeholder="No. kontrak..." />
                </div>
                <div>
                    <label style={labelStyle}>Kos (RM)</label>
                    <input type="number" step="0.01" style={inputStyle} value={data.cost} onChange={e => setData('cost', e.target.value)} placeholder="0.00" />
                </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                <div>
                    <label style={labelStyle}>Penerangan</label>
                    <input type="text" style={inputStyle} value={data.description} onChange={e => setData('description', e.target.value)} placeholder="Penerangan penyelenggaraan..." />
                </div>
                <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
                <button type="submit" disabled={processing} style={{ padding: '7px 20px', borderRadius: 6, border: 'none', background: '#C2410C', color: UTM.white, fontSize: 12, fontWeight: 700, cursor: processing ? 'not-allowed' : 'pointer', opacity: processing ? 0.7 : 1 }}>{processing ? 'Menyimpan...' : 'Tambah Penyelenggaraan'}</button>
...
function EditMaintenanceForm({ record, onDone }) {
    const { data, setData, put, processing, errors } = useForm({
        maintenance_date: record.maintenance_date ? record.maintenance_date.split('T')[0] : '',
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('assets.maintenances.update', [record.asset_id, record.id]), {
            preserveScroll: true,
            onSuccess: () => onDone(),
        });
    };
    ...
    return (
        <form onSubmit={handleSubmit} style={{ padding: 12, background: '#FFF7ED', borderRadius: 6, border: `1px solid #FED7AA`, marginBottom: 8 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: '#C2410C', marginBottom: 8 }}>Edit Penyelenggaraan</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
                <div><label style={labelStyle}>Tarikh</label><input type="date" style={inputStyle} value={data.maintenance_date} onChange={e => setData('maintenance_date', e.target.value)} required /></div>
                <div><label style={labelStyle}>Syarikat</label><input type="text" style={inputStyle} value={data.company_name} onChange={e => setData('company_name', e.target.value)} placeholder="Nama syarikat..." /></div>
                <div><label style={labelStyle}>No. Kontrak</label><input type="text" style={inputStyle} value={data.contract_no} onChange={e => setData('contract_no', e.target.value)} placeholder="No. kontrak..." /></div>
                <div><label style={labelStyle}>Kos (RM)</label><input type="number" step="0.01" style={inputStyle} value={data.cost} onChange={e => setData('cost', e.target.value)} placeholder="0.00" /></div>
                <div><label style={labelStyle}>Status</label><select style={inputStyle} value={data.status} onChange={e => setData('status', e.target.value)}><option value="pending">Pending</option><option value="completed">Completed</option></select></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                <div><label style={labelStyle}>Penerangan</label><input type="text" style={inputStyle} value={data.description} onChange={e => setData('description', e.target.value)} placeholder="Penerangan..." /></div>
                <div><label style={labelStyle}>Catatan</label><input type="text" style={inputStyle} value={data.notes} onChange={e => setData('notes', e.target.value)} placeholder="Catatan..." /></div>
            </div>
            {errors.description && <p style={{ fontSize: 11, color: '#DC2626', marginBottom: 8 }}>{errors.description}</p>}
            <div style={{ display: 'flex', gap: 8 }}>
...
export default function Kewpa13Index({ records, filters, assets }) {
    const [search, setSearch] = useState(filters?.search || '');
    const [statusFilter, setStatusFilter] = useState(filters?.status || '');
    const [showAdd, setShowAdd] = useState(false);
    const [editingId, setEditingId] = useState(null);

    const handleSearch = (e) => { e.preventDefault(); router.get(route('maintenances.index'), { search, status: statusFilter }, { preserveState: true, replace: true }); };
    const handleReset = () => { setSearch(''); setStatusFilter(''); router.get(route('maintenances.index'), {}, { preserveState: true, replace: true }); };

    const handleDelete = (record) => {
        if (window.confirm(`Padam rekod penyelenggaraan aset ${record.asset?.name} (${record.maintenance_date})?`)) {
            router.delete(route('assets.maintenances.destroy', [record.asset_id, record.id]), { preserveScroll: true });
        }
    };
    ...
    return (
        <AuthenticatedLayout>
            <Head title="KEW.PA-13/14 — Rekod Penyelenggaraan Aset" />
            <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
                <div style={{ marginBottom: 28 }}>
                    <h1 style={{ fontSize: 22, fontWeight: 800, color: UTM.maroon, margin: 0 }}>KEW.PA-13/14 — Rekod Penyelenggaraan Aset</h1>
                    <p style={{ fontSize: 13, color: UTM.gray500, marginTop: 4 }}>Senarai rekod penyelenggaraan aset</p>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', marginBottom: 16, flexWrap: 'wrap' }}>
                    <form onSubmit={handleSearch} style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap', flex: 1 }}>
                        <div style={{ flex: 1, minWidth: 200 }}>
                            ...
                            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Nama aset, tag, syarikat, kontrak..." style={inputStyle} />
                        </div>
                ...
                {showAdd && <AddMaintenanceForm assets={assets} onDone={() => setShowAdd(false)} />}
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', minWidth: '1050px', borderCollapse: 'collapse' }}>
                            <thead><tr style={{ background: UTM.gray50 }}>
                                ...
                                <th style={thStyle}>Syarikat</th>
                                <th style={thStyle}>Penerangan</th>
                                <th style={thStyle}>No. Kontrak</th>
                            ...
                            <tbody>
                                {records.data.length === 0 ? (
                                    <tr><td colSpan={8} style={{ ...tdStyle, textAlign: 'center', color: UTM.gray500 }}>Tiada rekod penyelenggaraan aset.</td></tr>
                                ) : records.data.map((record, idx) => (
                                    editingId === record.id ? (
                                        <tr key={record.id}>
                                            <td colSpan={8} style={{ padding: 0, background: '#FAFAFA' }}>
                                                <EditMaintenanceForm record={record} onDone={() => setEditingId(null)} />
                                            </td>
                                        </tr>
                                    ) : (
                                        <tr key={record.id} style={{ background: idx % 2 === 0 ? UTM.white : UTM.gray50, transition: 'background 0.12s' }}
                                            ...
                                            <td style={tdStyle}>{record.company_name || '—'}</td>
                                            <td style={{ ...tdStyle, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{record.description || '—'}</td>
                                            <td style={tdStyle}>{record.contract_no || '—'}</td>
                                            <td style={tdStyle}><StatusBadge status={record.status} /></td>
                                            <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>{record.maintenance_date ? new Date(record.maintenance_date).toLocaleDateString('ms-MY') : (record.created_at ? new Date(record.created_at).toLocaleDateString('ms-MY') : '—')}</td>
                                            <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>
                                                ...
                                                <a href={route('assets.maintenances.kewpa13.download', record.asset_id)}
