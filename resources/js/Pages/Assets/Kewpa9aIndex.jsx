...
export default function Kewpa9aIndex({ records, filters }) {
    const [search, setSearch] = useState(filters?.search || '');

    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('placements.index'), { search }, { preserveState: true, replace: true });
    };

    const handleReset = () => {
        setSearch('');
        router.get(route('placements.index'), {}, { preserveState: true, replace: true });
    };
                ...
                <form onSubmit={handleSearch} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 200 }}>
                        ...
                        <input
                            ...
                            placeholder="Nama aset, tag, peminjam, lokasi..."
                            style={inputStyle}
                        />
                    </div>
...
<table style={{ width: '100%', minWidth: '1050px', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ background: UTM.gray50 }}>
                                ...
                                <th style={thStyle}>No. Telefon</th>
                                <th style={thStyle}>Tarikh Pinjam</th>
                                <th style={thStyle}></th>
                            </tr>
                        </thead>
                        <tbody>
                            {records.data.length === 0 ? (
                                <tr>
                                    <td colSpan={7} style={{ ...tdStyle, textAlign: 'center', color: UTM.gray500 }}>
                                        Tiada rekod pinjaman / penempatan aset.
                                    </td>
                                </tr>
                                ...
                                >
                                    ...
                                    <td style={tdStyle}>{record.custodian_name || '—'}</td>
                                    <td style={tdStyle}>{record.location || '—'}</td>
                                    <td style={tdStyle}>{record.borrower_phone || '—'}</td>
                                    <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>
                                        {record.created_at ? new Date(record.created_at).toLocaleDateString('ms-MY') : '—'}
                                    </td>
                                    <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>
                                        <Link href={route('assets.placements.kewpa9a', [record.asset_id, record.id])}
                                            style={{
                                        ...
                                        <a href={route('assets.placements.kewpa9a.download', [record.asset_id, record.id])}
                                            style={{
                                                display       : 'inline-block',
                                                padding       : '5px 12px',
                                                borderRadius  : 6,
