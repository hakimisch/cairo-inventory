...
export default function Kewpa23({ sale }) {
    return (
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-gray-800">KEW.PA-23 — Lelongan Jualan Aset</h2>
                    <div className="flex gap-2">
                        <Link href={route('disposal-sales.show', sale.id)} className="text-sm text-indigo-600 hover:text-indigo-900">← Back</Link>
                        <Link href={route('disposal-sales.kewpa23.download', sale.id)} className="px-3 py-1.5 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700">Download PDF</Link>
                    </div>
                </div>
            }
        >
            <Head title="KEW.PA-23 - Lelongan Jualan Aset" />
            <div className="py-6">
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h3 className="text-lg font-semibold text-center mb-6">KEW.PA-23 — LELONGAN JUALAN ASET</h3>
                        <div className="text-sm space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div><span className="text-gray-500">Rujukan:</span> <span className="font-medium">{sale.sale_reference}</span></div>
                                <div><span className="text-gray-500">Tarikh Lelongan:</span> <span className="font-medium">{sale.sale_date ?? '-'}</span></div>
                            ...
                            {sale.deposit_required && <div><span className="text-gray-500">Deposit Diperlukan:</span> <span className="font-medium">RM {Number(sale.deposit_required).toFixed(2)}</span></div>}

                            <h4 className="font-semibold mt-4 mb-2">Lot Lelongan</h4>
                            <table className="w-full border-collapse border border-gray-300 text-xs">
                                <thead><tr className="bg-gray-100">
                                    ...
                                    <th className="border border-gray-300 p-2">Harga Rizab (RM)</th>
                                </tr></thead>
                                <tbody>
                                    {sale.disposal_sale_items?.map(item => (
                                        <tr key={item.id}>
                                            ...
                                            <td className="border border-gray-300 p-2 text-right">{item.reserve_price ? Number(item.reserve_price).toFixed(2) : '-'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
