...
<body class="p-8 text-[10px] leading-tight bg-white font-serif text-black">
    ...
    <div class="flex justify-between items-start mb-1">
        ...
        <div class="text-right">
            <div class="font-bold text-[14px]">KEW.PA-4</div>
        </div>
    </div>

    <div class="text-center font-bold text-[14px] mb-1 uppercase">UNIVERSITI TEKNOLOGI MALAYSIA</div>
    <div class="text-center font-bold text-[13px] mb-6 uppercase">SENARAI DAFTAR HARTA TETAP</div>
    ...
    <table class="w-full border-collapse border border-black text-center">
        <thead class="bg-gray-50 font-bold">
            <tr>
                ...
                <th class="border border-black p-1.5 w-20">Tarikh Terima</th>
                <th class="border border-black p-1.5 w-24">Harga Perolehan (RM)</th>
        ...
        <tbody>
            @forelse($assets as $index => $asset)
                <tr class="h-8">
                    ...
                    <td class="border border-black p-1.5">{{ $asset->received_date ? \Carbon\Carbon::parse($asset->received_date)->format('d/m/Y') : '-' }}</td>
                    <td class="border border-black p-1.5 text-right">{{ number_format($asset->purchase_price, 2) }}</td>
                    <td class="border border-black p-1.5 uppercase">{{ $asset->status ?? 'AKTIF' }}</td>
                </tr>
            @empty
                <tr class="h-8">
                    <td class="border border-black p-1.5 italic text-gray-500" colspan="8">Tiada harta tetap dijumpai untuk tahun ini.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="mt-4 text-right font-bold text-[11px]">
        Jumlah Aset: {{ $assets->count() }} &nbsp;|&nbsp; Jumlah Nilai: RM {{ number_format($assets->sum('purchase_price'), 2) }}
    </div>
    ...
    <div class="mt-6 text-[9px] italic border-t border-gray-400 pt-3 text-center">
        * Laporan ini dijana secara automatik daripada sistem CAIRO Inventory. KEW.PA-4 — Senarai Daftar Harta Tetap
    </div>
