...
<body>
    ...
    <div class="ref-line">
        <strong>Ruj. Kami :</strong> UTM.CAIRO/PA-29/{{ $lossReport->id }}/{{ date('Y') }}<br>
        <strong>Tarikh &nbsp;&nbsp;&nbsp;:</strong> {{ \Carbon\Carbon::now()->format('d F Y') }}
    </div>

    <div class="title-line">SURAT PELANTIKAN JAWATANKUASA PENYIASAT KEHILANGAN ASET ALIH UNIVERSITI</div>
    <div class="title-line" style="font-size: 10px; text-decoration: none;">(KEW.PA-29)</div>
    ...
    <table>
        ...
        <tbody>
            @php
                $committee = $lossReport->committeeAppointments ?? collect();
            @endphp
    ...
    <div class="body-text">
        ...
        <strong>Jawatankuasa Penyiasat Kehilangan Aset Alih Universiti</strong>
        bagi tujuan menyiasat kehilangan aset yang berikut:
    </div>

    <table>
        <tr>
            <th style="width:25%">No. Siri Pendaftaran</th>
            <td class="left">{{ $lossReport->asset->asset_tag ?? '—' }}</td>
        </tr>
        <tr>
            <th>Nama Aset</th>
            <td class="left">{{ $lossReport->asset->name ?? '—' }}</td>
        </tr>
        <tr>
            <th>Tarikh Kehilangan</th>
            <td class="left">{{ $lossReport->loss_date ? \Carbon\Carbon::parse($lossReport->loss_date)->format('d/m/Y') : '—' }}</td>
        </tr>
        <tr>
            <th>Tempat Kehilangan</th>
            <td class="left">{{ $lossReport->incident_location ?? '—' }}</td>
        </tr>
        <tr>
            <th>No. Laporan Polis</th>
            <td class="left">{{ $lossReport->police_report_no ?? '—' }}</td>
        </tr>
        <tr>
            <th>Anggaran Nilai Semasa</th>
            <td class="left">RM {{ number_format($lossReport->current_value ?? 0, 2) }}</td>
        </tr>
    </table>

    <div class="body-text">
        Tuan/puan adalah diminta menjalankan siasatan dengan penuh tanggungjawab dan mengemukakan laporan akhir siasatan kepada pihak pengurusan Universiti dalam tempoh yang ditetapkan.
    </div>
    ...
    <div class="signature-block">
        <div class="signature-row">
            <div class="signature-box">
                <div class="sig-line">{{ $lossReport->committeeAppointments->firstWhere('role', 'chairman')?->user?->name ?? '____________________' }}</div>
                <div class="sig-role">Pengerusi</div>
                <div class="sig-role">Jawatankuasa Penyiasat Kehilangan</div>
