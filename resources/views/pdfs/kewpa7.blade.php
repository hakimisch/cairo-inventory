...
<body class="p-8 text-[10px] leading-tight bg-white font-serif text-black">
    ...
    <div class="text-center font-bold text-[13px] mb-6 uppercase">LAPORAN KEDUDUKAN ASET</div>
    ...
    <table class="w-full border-collapse border border-black text-center">
        <thead class="bg-gray-50 font-bold">
            <tr>
                ...
                <th class="border border-black p-1.5">Lokasi Semasa</th>
        ...
        <tbody>
            @forelse($assets as $index => $asset)
                <tr class="h-8">
                    ...
                    <td class="border border-black p-1.5 uppercase">{{ $asset->custodian_name ?? '-' }}</td>
                    <td class="border border-black p-1.5 text-right">{{ number_format($asset->purchase_price, 2) }}</td>
                    <td class="border border-black p-1.5 uppercase">{{ $asset->status }}</td>
                </tr>
            @empty
                <tr class="h-8">
                    <td class="border border-black p-1.5 italic text-gray-500" colspan="8">Tiada aset dijumpai untuk lokasi ini.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
